...
r = [1.8]
tot_iter = 5
E_casci = np.zeros((len(r), tot_iter))      # array of output data
E_tccsd = np.zeros((len(r), tot_iter))      # array of output data
entropy = np.zeros((len(r), tot_iter))      # array of output data
...
for i in range(len(r)):
    ...
    my_qio = QIO(mf=mf, sol=my_dmrg, act_space=act_space) 
    my_qio.max_cycle = 4
    ...
    for micro_i in range(tot_iter):
        ...
        E_dmrg[i, micro_i] = my_qio.sol.e_tot

        entropy[i, micro_i] = get_cost_fqi(my_qio.gamma, my_qio.Gamma, inactive_indices)
        #entropy_tccsd[i, micro_i] = get_cost_fqi(gamma, Gamma, inactive_indices)
        np.savetxt('c2_dmrg_energy_'+basis+'.8in8.txt',E_dmrg)
        np.savetxt("c2_dmrg_entropy_"+basis+".8in8.txt", np.asarray(entropy))
