...
class DMRGCI(lib.StreamObject):
    ...
    def make_rdm12(self, state, norb, nelec, link_index=None, spin=False, **kwargs):
        nelectrons = 0
        if isinstance(nelec, (int, numpy.integer)):
          nelectrons = nelec
        else:
        ...
        twopdms = numpy.zeros( (norb, norb, norb, norb) )
        file2pdm = "spatial_twopdm.%d.%d.txt" %(state, state)
        file2pdms = "spatial_twopdms.%d.%d.txt" %(state, state) # 2RDM with indices (iup,lup,jdown,kdown)
        ...
        with open(os.path.join(self.scratchDirectory, "node0", file2pdms), "r") as f:
            norb_read = int(f.readline().split()[0])
            assert(norb_read == norb)
            for line in f:
                linesp = line.split()
                i, k, l, j = [int(x) for x in linesp[:4]]
                twopdms[i,j,k,l] = 2.0 * float(linesp[4])
        ...
        if spin==False:
            ...
            # when spin argument is true, returns the 2RDM part with indices (iup,lup,jdown,kdown)
            return onepdm, twopdms
